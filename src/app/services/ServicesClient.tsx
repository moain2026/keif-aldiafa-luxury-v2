...
function ServiceModal({ service, onClose }: { service: ServiceItem; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-center justify-center p-2 sm:p-4" onClick={onClose} role="dialog" aria-modal="true" aria-label={`تفاصيل خدمة ${service.title}`}>
      <div className="absolute inset-0 bg-black/80 backdrop-blur-md" />
      <motion.div initial={{ opacity: 0, scale: 0.92, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 30 }} transition={{ type: "spring", damping: 25, stiffness: 250 }} onClick={(e) => e.stopPropagation()} className="relative w-[96%] sm:max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl" style={{ background: "linear-gradient(160deg, rgba(25,20,8,0.98), rgba(15,12,5,0.99))", border: "1px solid rgba(184,134,11,0.25)", boxShadow: "0 40px 80px rgba(0,0,0,0.8)" }}>
        ...
        <div className="relative w-full aspect-[4/3] sm:aspect-[16/10] overflow-hidden rounded-t-3xl">
          <ImageWithFallback
        ...
        <div className="p-5 sm:p-6">
          <p className="text-[#F5F5DC]/65 text-sm leading-relaxed mb-4">{service.description}</p>
          ...
          {service.outfits.length > 0 && (
            <div className="mb-4">
              <h3 className="text-[#B8860B] text-xs mb-2" style={{ fontWeight: 700 }}>الأزياء المتاحة</h3>
              <div className="flex gap-2 overflow-x-auto scroll-hide pb-1">
                {service.outfits.map((o, i) => (
                  <button key={i} onClick={() => setSelectedOutfit(i)} className="flex-shrink-0 w-20 rounded-lg overflow-hidden transition-all duration-300" style={{ border: selectedOutfit === i ? "2px solid #B8860B" : "2px solid rgba(184,134,11,0.1)", opacity: selectedOutfit === i ? 1 : 0.6 }}>
                    <ImageWithFallback src={o.img} alt={o.name} className="w-full h-16 object-cover" />
                    <p className="text-center text-[10px] py-0.5 text-[#F5F5DC]/60">{o.name}</p>
                  </button>
                ))}
              </div>
            </div>
